...
Storage.prototype.onappend = function (filename, st, cb) {
  ...
  this._limitCache(function retry (err) {
    ...
    self.cache.push({
      ...
      storage: raf(filename, {directory: self.directory})
    })
...
function bisect (feed, search, cb) {
  ...
  function loop (err, entry) {
    ...
    if (!entry.value) {
      // Inflate entry
      findNonInflateEntryBefore(mid, function (err, index) {
        if (err) return cb(err)
        if (index) {
          mid = index
          return feed.get(mid, {valueEncoding: Entry}, loop)
        }
        findNonInflateEntryAfter(mid, function (err, index) {
          if (err) return cb(err)
          if (index) {
            mid = index
            return feed.get(mid, {valueEncoding: Entry}, loop)
  ...
  function findNonInflateEntryBefore (index, cb) {
    const testIndex = index - 1
    if (testIndex <= btm) return cb() // No match
    feed.get(testIndex, {valueEncoding: Entry}, function (err, entry) {
      if (err) return cb(err) 
      if (entry.value) return cb(null, testIndex)
      findNonInflateEntryBefore(testIndex, cb) 
    })
  }

  function findNonInflateEntryAfter (index, cb) {
    const testIndex = index + 1
    if (testIndex >= top) return cb() // No match
    feed.get(testIndex, {valueEncoding: Entry}, function (err, entry) {
      if (err) return cb(err) 
      if (entry.value) return cb(null, testIndex)
      findNonInflateEntryAfter(testIndex, cb) 
    })
  }
